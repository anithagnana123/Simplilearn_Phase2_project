...
import simplilearn.model.ClassSubTeach;
import simplilearn.model.Student;
import simplilearn.model.SubInfo;
...
try {	
  ...
  // add a subject
     if(operation.equals("addsubject")) {
    	...
    	String subject = request.getParameter("subject");
 		...
 		session.beginTransaction();
 		SubInfo thesubject = SubInfo.getInstance();

 		thesubject.setSubject(subject);
 	
 		session.save(thesubject);
 		session.getTransaction().commit();
 		
 	    out.println("<h2> Subject added successfully </h2>");
 		
 		out.println("<form action='addsubject' method='get'>");
 	  	out.println("<br> <input type='submit' value='Add another subject'/>");
 	    out.println("</form>");
 	
      }

     // delete a class 
     if(operation.equals("deletesubject")) {
		...
		thesubject.setSub_id(id);
		
		session.delete(thesubject);
		session.getTransaction().commit();
		
	    out.println("<h2> Subject deleted successfully </h2>");
		
		out.println("<form action='deletesubject' method='get'>");
	  	out.println("<br> <input type='submit' value='Delete another subject'/>");
	    out.println("</form>");
     ...
     if(operation.equals("updatestudent")) {
    	...
    	String student_id = request.getParameter("student_id");
 		...
 		out.println("<form action='updatestudent.jsp' method='get'>");
  ...
  // assign teacher to a class and subject  
     if(operation.equals("classsubteach")) {
		...
		session.beginTransaction();
		ClassSubTeach classsubteach = ClassSubTeach.getInstance();
		classsubteach.setTeacher_id(teach_id);
		
		ClassSubData classsub = ClassSubData.getInstance();
		classsub.setSubject(subject);
		classsub.setClass_sec(class_sec);
		classsubteach.setClasssub(classsub);
		
		session.save(classsubteach);
		session.getTransaction().commit();
		
	    out.println("<h2> Teacher assigned to a class and subject successfully </h2>");
		
		out.println("<form action='classsubteach.jsp' method='get'>");
	  	out.println("<br> <input type='submit' value='Add another teacher to a class and subject'/>");
	    out.println("</form>");
